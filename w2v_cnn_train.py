...
path = './data'
output_path = './w2v_cnn_weibo_output/'
...
class Model(torch.nn.Module):
    def __init__(self,vocab=TEXT.vocab):
        ...
        self.chan_num2 = 128
        self.chan_num3 = 64
        self.insize = self.chan_num3*10  # 注意check
        self.outsize = 2
        self.vocab = vocab

        self.embed = torch.nn.Embedding(num_embeddings=len(vocab), embedding_dim=self.embed_dim)
        self.embed.weight.data.copy_(vocab.vectors)
        self.conv1 = torch.nn.Conv1d(in_channels=self.embed_dim, out_channels=self.chan_num1, kernel_size=5, padding=0)
        self.conv2 = torch.nn.Conv1d(in_channels=self.chan_num1, out_channels=self.chan_num2, kernel_size=5, padding=0)
        self.conv3 = torch.nn.Conv1d(in_channels=self.chan_num2, out_channels=self.chan_num3, kernel_size=3, padding=0)
    ...
    def forward(self, rev, lab):
        ...
        rev = self.pool2(rev)
        rev = self.conv3(rev)
        rev = rev.permute(0, 2, 1).flatten(1)
        rev = self.fc(rev)
        rev = self.tanh(rev)
...
def train(Model,modelname,lr=5e-4):
    ...
    model_path = output_path+'/w2v_cnn_{}.model'.format(modelname)
...
def test(model,modelname):
    ...
    result = str(report)
    print(result)
    with open(output_path+'train_w2v_cnn_{}_result.txt'.format(modelname),'w',encoding='utf-8') as f:
        f.write(result)

...
path = './data'
output_path = './w2v_dnn_weibo_output/'
...
class Model(torch.nn.Module):
    def __init__(self,vocab=TEXT.vocab):
        ...
        self.maxlen = 128
        self.embed_dim = 100
        self.insize = self.maxlen*self.embed_dim
        self.outsize = 2
        self.vocab = vocab
...
def train(Model,modelname,lr=5e-4):
    ...
    model_path = output_path+'/w2v_dnn_{}.model'.format(modelname)
...
def test(model,modelname):
    ...
    result = str(report)
    print(result)
    with open(output_path+'train_w2v_dnn_{}_result.txt'.format(modelname),'w',encoding='utf-8') as f:
        f.write(result)

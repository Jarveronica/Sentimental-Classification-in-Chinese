...
path = './data'
output_path = './w2v_lsvc_weibo_output/'
...
def train(Model,modelname):
    ...
    model0 = Model()
    model0.to(device)
    model = SGDClassifier(loss='hinge', warm_start=True,l1_ratio=0.4)  # loss='hinge'即为linearSVC
    ...
    for epoch in range(epoch_n):
        ...
        for batch, (rev, lab) in enumerate(loader):
            ...
            # loss = loss_func(torch.tensor(model.predict_proba(rev)),lab) 不存在loss
            ...
            if batch % 10 == 0:
                print('batch:%d, accu:%.8f' % (batch, accu_sum))

                if batch % (batch_n*10) == 0:  # 只训练batch_n*10个batch
                    break
        test(model0,model,modelname)

    joblib.dump(model, output_path + 'weibo_lsvc_{}_model.pkl'.format(modelname))
...
def test(model0,model,modelname):
    ...
    result = str(report)
    print(result)
    with open(output_path+'train_w2v_lsvc_{}_result.txt'.format(modelname),'w',encoding='utf-8') as f:
        f.write(result)
